...
# Remove commas and cast population columns to float
...
df = df.merge(df_pop[['state', 'population']],
              on=['state'],
              how='left')
...
# Prepare Data
df = df.dropna(subset=['so2', 'no2', 'rspm'])  # drop if target pollutants missing
...
X = pd.get_dummies(X, columns=['type', 'state', 'location', 'month', 'year', 'location_monitoring_station','population'])
